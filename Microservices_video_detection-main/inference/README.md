...
#### 2. run_in_executor для CPU-интенсивных задач (YOLO детекция)

**Проблема**: YOLO детекция — CPU-интенсивная операция, которая блокирует GIL и event loop.

**Решение**: Использование `run_in_executor` для вынесения YOLO обработки в отдельный поток.
...
# Запускаем YOLO на кадре в отдельном потоке
...
- YOLO обработка выполняется в отдельном потоке
- Event loop продолжает обрабатывать другие задачи
- Когда обработка завершается, результат возвращается через await

**Примечание**: Для ещё большей производительности можно использовать `ProcessPoolExecutor`, но в текущей реализации используется `ThreadPoolExecutor`, так как YOLO модель уже оптимизирована (Ultralytics использует PyTorch, который эффективно использует CPU).
...
def update_outbox(event_id, status):
    """Синхронная функция для обновления статуса"""
...
await loop.run_in_executor(None, update_outbox, event_id, "in progress")
```
...
**Решение**:
1. **ThreadPoolExecutor** (через `run_in_executor(None, ...)`) для YOLO обработки:
   - YOLO модель использует PyTorch, который эффективно использует CPU
   - Event loop не блокируется
   - Можно обрабатывать несколько кадров параллельно (несколько потоков)

2. **ThreadPoolExecutor** для I/O операций:
   - MongoDB операции
   - Event loop не блокируется

**Примечание**: Для ещё большей производительности можно использовать `ProcessPoolExecutor`, но это требует сериализации модели между процессами, что может быть неэффективно. Текущая реализация с `ThreadPoolExecutor` обеспечивает хорошую производительность благодаря оптимизациям PyTorch.

**Пример использования**:
```python
# CPU-интенсивная задача (YOLO) — используем ThreadPoolExecutor
loop = asyncio.get_running_loop()
frame_result = await loop.run_in_executor(
...
Сервис обновляет статусы в MongoDB:
- `in progress` — кадр обрабатывается inference сервисом
...
1. **Производительность**: Модель кэшируется после первой загрузки
2. **Надёжность**: Отключён авто-коммит Kafka для гарантированной обработки
3. **Масштабируемость**: Можно запускать несколько экземпляров
