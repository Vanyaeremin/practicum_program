...
1. **Начинайте с общего просмотра**: Используйте `docker-compose logs` чтобы увидеть общую картину
2. **Используйте tail для последних событий**: `--tail=100` показывает последние 100 строк
3. **Следите в реальном времени**: Используйте `-f` для отслеживания проблем по мере их возникновения
4. **Фильтруйте по ключевым словам**: Используйте `grep` для поиска конкретных ошибок или событий
5. **Сохраняйте важные логи**: Перенаправляйте вывод в файл для дальнейшего анализа

## Структура логов

Каждый сервис логирует свою деятельность:
- **api_checkout**: HTTP запросы, ответы, ошибки API
- **inference**: Обработка видео, детекция объектов, метрики производительности
- **outbox**: Очередь задач, обработка результатов
- **outbox_s3**: Загрузка файлов в S3, операции с хранилищем
- **runner**: Координация сервисов, управление задачами
