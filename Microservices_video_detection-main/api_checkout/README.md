...
### Статусы обработки
- `new` — Новое событие создано, ожидает отправки в очередь
- `sent` — Событие отправлено в очередь обработки
- `in progress` — Видео обрабатывается в сервисах runner/inference
- `have ready result` — Детекция завершена, результат готовится к сохранению
- `predicted` — Результат готов и доступен для получения
