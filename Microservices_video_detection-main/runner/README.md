...
9. **Устанавливает статус** `predictable` в runner_db и `have ready result` в main_db.outbox
...
# При завершении обновляем статусы
# В runner_db: status = "predictable"
# В main_db.outbox: status = "have ready result"
```
...
### В main_db.outbox:
- `in progress` — начало обработки в runner
- `have ready result` — все кадры обработаны, результат готов к сохранению

### В runner_db.runner_predictions:
- `processing` — идёт обработка кадров
- `predictable` — все кадры обработаны, результат готов для outbox_s3
