...
4. **Обновление статуса** в runner_db на `"ready"` (НЕ обновляет outbox - соответствует принципам микросервисов)
...
5. **Микросервисная архитектура**: Outbox S3 Worker НЕ обновляет outbox - только runner_db
...
2. **Поиск данных**: Outbox S3 Worker ищет записи напрямую в `runner_db.runner_predictions` со статусом `"predictable"`
3. **Чтение данных**: Из записи в runner_db берутся `event_id`, `bucket` и `prediction` (финальный JSON)
4. **Сохранение**: Worker сохраняет prediction JSON в MinIO по пути `detection/{event_id}.json`
5. **Завершение**: После успешного сохранения статус обновляется на `"ready"` в runner_db (НЕ в outbox)
6. **Сохранение истории**: Данные остаются в `runner_db` (не удаляются) для аналитики
7. **Микросервисная архитектура**: Outbox S3 Worker НЕ обновляет outbox - только runner_db
...
**ВАЖНО**: Outbox S3 Worker НЕ обновляет outbox (main_db.outbox) - только runner_db. Это соответствует принципам микросервисной архитектуры.
...
while True:
    ...
    for runner_pred in ready_predictions:
        ...
        # Обновление статуса в runner_db (НЕ в outbox - соответствует принципам микросервисов)
        runner_predictions.update_one(
            {"_id": event_id, "status": "predictable"},  # Условие для идемпотентности
            {
                "$set": {
                    "status": "ready",  # Результат готов к забору из S3
...
### Обновление в runner_db (НЕ в outbox)
```json
{
  "_id": "video-uuid",
  "status": "ready",
...
**ВАЖНО**: Outbox S3 Worker НЕ обновляет outbox - только runner_db. Это соответствует принципам микросервисной архитектуры.
...
- **Обработка**: Логирование ошибки, запись остаётся со статусом `"predictable"` в runner_db
- **Восстановление**: Запись будет обработана при следующей итерации
...
1. **Отсутствие идемпотентности**: При повторной обработке записи со статусом "predictable" файл мог быть перезаписан в S3, даже если он уже существовал.

2. **Race condition**: При параллельной работе нескольких экземпляров воркера одна и та же запись могла быть обработана одновременно, что приводило к дублированию файлов в S3 или конфликтам при обновлении статуса.

3. **Проблема атомарности между S3 и MongoDB**: Если `put_object()` успешен, но `update_one()` упал (например, сеть упала, MongoDB недоступен), файл уже был в S3, но статус оставался "predictable". При следующей итерации файл мог быть перезаписан или обработан повторно.

4. **Нет защиты от изменения статуса**: Обновление статуса не проверяло, что статус всё ещё "predictable", что могло привести к обновлению уже обработанных записей.
...
4. **Проверка результата обновления**: После обновления проверяется `result.modified_count` для определения, было ли обновление успешным или запись уже была обработана.

**Цель исправлений:**

- **Идемпотентность**: Гарантировать, что повторная обработка записи не создаст дубликаты файлов в S3
- **Защита от race condition**: Предотвратить параллельную обработку одной записи несколькими воркерами
- **Восстановление после сбоев**: Если `put_object()` успешен, но `update_one()` упал, при следующей итерации файл будет найден в S3, и статус обновится без повторного сохранения
...
# Проверка существования файла в S3 (идемпотентность)
file_exists = False
try:
...
Полной атомарности между MinIO (S3) и MongoDB достичь невозможно, так как это разные распределённые системы. Однако исправления обеспечивают идемпотентность операций и устойчивость к сбоям, что является практическим решением для распределённых систем.
