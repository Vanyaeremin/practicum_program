...
6. **Обновление статуса**: `sent` → `in progress`
7. **Статус**: `in progress` — Видео обрабатывается в runner
...
6. **Статус**: `in progress` — Кадры обрабатываются в inference
...
5. **Обновление статуса**: 
   ...
   - В main_db.outbox: `in progress` → `have ready result`
6. **Статус**: `have ready result` — Детекция завершена, результат готов
...
6. **Статус**: `predicted` — Результат готов и доступен
...
2. **Проверка статуса**: Чтение статуса из MongoDB outbox
3. **Загрузка из MinIO**: JSON загружается из `detection/{video_id}.json`
4. **Возврат результата**: JSON с детекциями и временем обработки

## Система статусов

### Полный жизненный цикл
1. **`new`** — Новое событие создано, ожидает отправки в очередь
2. **`sent`** — Событие отправлено в очередь обработки
3. **`in progress`** — Видео обрабатывается в сервисах runner/inference
4. **`have ready result`** — Детекция завершена, результат готовится к сохранению
5. **`predicted`** — Результат готов и доступен для получения

### Дополнительные статусы (внутренние)
- **`processing`** — Кадры обрабатываются в `runner_db.runner_predictions`
- **`predictable`** — Результат готов к сохранению в S3 (в runner_db)
...
```python
# inference/kafka_consumer.py — обновление статуса в MongoDB
loop = asyncio.get_running_loop()
def update_outbox():
    client = MongoClient(MONGO_URI)
    db = client['main_db']
    outbox = db['outbox']
    outbox.update_one({"_id": event_id}, {"$set": {"status": "in progress"}})
    client.close()

await loop.run_in_executor(None, update_outbox)
```
...
while True:
    ...
    for event in events:
        try:
            ...
            # Отправляем в Kafka тему "outbox_events"
...
while True:
    try:
        ...
        for runner_pred in ready_predictions:
            ...
            try:
                # Сохраняем prediction JSON в MinIO
                ...
                # Обновляем статус только после успешного сохранения
                outbox.update_one(
                    {"_id": event_id},
                    {
                        "$set": {
                ...
                # При ошибке событие остаётся со статусом "have ready result"
                # Будет обработано при следующей итерации
...
- **Сохранение состояния**: Событие остаётся со статусом "have ready result" при ошибке
- **Автоматическое восстановление**: При следующей итерации событие будет обработано снова
...
5. **Готовность**: Runner устанавливает статус `"predictable"` в runner_db и `"have ready result"` в outbox
6. **Сохранение**: Outbox S3 Worker находит запись в runner_db со статусом `"predictable"` и сохраняет результат в MinIO
7. **Завершение**: После успешного сохранения статус обновляется на `"predicted"` в outbox
  ...
  "operations": [
    {
      ...
      "status_description": "Результат готов и доступен",
      "processing_time_seconds": 45,
      "processing_time_formatted": "45s"
    }
  ],
